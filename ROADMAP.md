# Cubeforge Examples Roadmap

This is the development roadmap for **cubeforge-examples** -- a growing collection of example games built with the [Cubeforge](https://github.com/1homsi/cubeforge) game engine. The repo currently ships **64 playable examples** and is on track to reach **100+ games** spanning every genre a 2D engine can handle.

---

## Legend

| Icon | Meaning |
|------|---------|
| :white_check_mark: | **Done** -- playable and merged |
| :white_check_mark: | **In Progress** -- actively being built |
| :white_circle: | **Planned** -- open for contribution |

---

## Platformers

| Game | Status | Description |
|------|--------|-------------|
| Platformer | :white_check_mark: | Classic side-scrolling platformer |
| Endless Runner | :white_check_mark: | Auto-scrolling run-and-jump game |
| Mario Clone | :white_check_mark: | Mario-inspired level-based platformer |
| Metroidvania | :white_check_mark: | Exploration with ability-gated areas |
| Precision Platformer | :white_circle: | Celeste-style tight movement mechanics |
| Auto-runner | :white_check_mark: | One-button auto-scrolling runner |
| Rope / Grappling Hook | :white_circle: | Swing and launch with rope physics |
| Parkour | :white_circle: | Momentum-based acrobatic movement |
| Jetpack | :white_circle: | Thrust-based vertical platforming |
| Wall-jump | :white_check_mark: | Wall-cling and wall-jump mechanics |

## Arcade

| Game | Status | Description |
|------|--------|-------------|
| Snake | :white_check_mark: | Classic snake with growing tail |
| Pong | :white_check_mark: | Two-paddle ball bouncing game |
| Breakout | :white_check_mark: | Paddle and brick destruction |
| Flappy Bird | :white_check_mark: | Tap-to-fly through pipe gaps |
| Asteroids | :white_check_mark: | Ship rotation and asteroid shooting |
| Tetris | :white_check_mark: | Falling block stacking puzzle |
| Pac-Man | :white_check_mark: | Maze navigation with ghost AI |
| Space Invaders | :white_check_mark: | Rows of aliens descending slowly |
| Frogger | :white_check_mark: | Cross traffic and river safely |
| Arkanoid | :white_check_mark: | Enhanced breakout with power-ups |
| Qix | :white_circle: | Claim territory by drawing lines |
| Missile Command | :white_check_mark: | Defend cities from falling missiles |
| Galaga | :white_check_mark: | Formation-based space shooting arcade |
| Centipede | :white_circle: | Shoot segmented centipede in garden |
| Dig Dug | :white_circle: | Dig tunnels and inflate enemies |
| Lunar Lander | :white_check_mark: | Thrust-controlled precision moon landing |

## Puzzle

| Game | Status | Description |
|------|--------|-------------|
| 2048 | :white_check_mark: | Slide and merge numbered tiles |
| Minesweeper | :white_check_mark: | Uncover grid avoiding hidden mines |
| Sokoban | :white_check_mark: | Push boxes onto target squares |
| Lights Out | :white_check_mark: | Toggle lights to clear the board |
| Match-3 | :white_check_mark: | Swap gems to match three |
| Tetris Puzzle | :white_circle: | Shape-fitting spatial logic puzzles |
| Dr. Mario | :white_check_mark: | Match-color virus elimination game |
| Puyo Puyo | :white_check_mark: | Chain-reaction blob matching puzzle |
| Sliding Puzzle | :white_check_mark: | Rearrange tiles into correct order |
| Pipe Puzzle | :white_check_mark: | Connect pipes to complete flow |
| Laser Mirror Puzzle | :white_circle: | Redirect laser beams with mirrors |
| Block Push Puzzles | :white_circle: | Advanced block-pushing logic challenges |
| Gravity Puzzles | :white_circle: | Shift gravity to solve levels |
| Logic Grid Puzzles | :white_circle: | Deduction puzzles on a grid |
| Sokoban Variants | :white_circle: | Extended Sokoban with new mechanics |

## Top-down Action

| Game | Status | Description |
|------|--------|-------------|
| Dungeon Explorer | :white_check_mark: | Explore procedural dungeon rooms |
| RPG | :white_check_mark: | Top-down RPG with dialogue system |
| Roguelike | :white_check_mark: | Procedural permadeath dungeon crawler |
| Twin-Stick Shooter | :white_check_mark: | Dual-stick directional combat arena |
| Vampire Survivors Style | :white_check_mark: | Auto-attack horde survival game |
| Zelda-like Adventure | :white_circle: | Overworld exploration with item puzzles |
| Survival Arena | :white_circle: | Last-man-standing arena combat |
| Stealth Game | :white_circle: | Avoid detection with line-of-sight |
| Arena Combat | :white_circle: | Fast-paced melee arena fighting |
| Boss Rush | :white_circle: | Back-to-back epic boss battles |

## Shooters

| Game | Status | Description |
|------|--------|-------------|
| Space Shooter | :white_check_mark: | Horizontal scrolling space combat |
| Bullet Hell | :white_check_mark: | Dense bullet patterns to dodge |
| Vertical Shooter | :white_check_mark: | Classic top-down vertical shooting |
| Side-scroll Shooter | :white_check_mark: | Run-and-gun side-scrolling action |
| Arena Shooter | :white_circle: | 360-degree shooting in closed arena |
| Tank Shooter | :white_circle: | Armored vehicle combat game |
| Turret Defense Shooter | :white_circle: | Stationary turret wave defense |
| Mech Shooter | :white_circle: | Pilotable mech suit combat |
| Boss Bullet Hell | :white_circle: | Single-boss bullet hell encounters |
| Rail Shooter | :white_circle: | On-rails auto-scrolling target shooting |

## Strategy

| Game | Status | Description |
|------|--------|-------------|
| Tower Defense | :white_check_mark: | Place towers to stop enemy waves |
| RTS-lite | :white_circle: | Simplified real-time strategy game |
| Turn-based Strategy | :white_check_mark: | Grid-based tactical turn combat |
| Auto Battler | :white_circle: | Place units and watch them fight |
| Tactical Grid Combat | :white_circle: | Fire Emblem-style positional battles |
| Base Defense | :white_circle: | Build and defend a home base |
| Resource Management | :white_circle: | Gather and allocate scarce resources |
| Civilization-lite | :white_circle: | Simplified empire-building strategy |
| War Simulation | :white_circle: | Large-scale army movement simulation |
| Economy Sim | :white_circle: | Supply, demand, and trade management |

## Simulation

| Game | Status | Description |
|------|--------|-------------|
| Idle / Incremental | :white_check_mark: | Click and automate for progression |
| City Builder | :white_check_mark: | Zone and build a growing city |
| Farming Sim | :white_check_mark: | Plant, water, harvest crop cycles |
| Factory Builder | :white_circle: | Automate production with conveyor belts |
| Colony Sim | :white_circle: | Manage colonists and their needs |
| Life Sim | :white_circle: | Simulate daily life decisions |
| Pet Sim | :white_circle: | Raise and care for virtual pets |
| Theme Park Builder | :white_circle: | Design rides and manage guests |
| Traffic Simulation | :white_circle: | Model and optimize traffic flow |
| Ecosystem Simulation | :white_circle: | Predator-prey population dynamics |

## Multiplayer

| Game | Status | Description |
|------|--------|-------------|
| Multiplayer | :white_check_mark: | Networked multiplayer demo game |
| Local Co-op Dungeon | :white_circle: | Shared-screen cooperative dungeon crawl |
| PvP Arena | :white_circle: | Player-versus-player combat arena |
| Capture the Flag | :white_circle: | Team-based flag capture game |
| Agar.io Style | :white_circle: | Grow by consuming smaller players |
| Slither.io Style | :white_circle: | Snake-like competitive multiplayer |
| Mini Battle Royale | :white_circle: | Shrinking zone last-player-standing |
| Soccer / Hockey | :white_circle: | Physics-based sports with goals |
| Platform Race | :white_circle: | Competitive side-scrolling race |
| Party Mini-games | :white_circle: | Collection of quick party rounds |

## Racing

| Game | Status | Description |
|------|--------|-------------|
| Top-down Racing | :white_circle: | Overhead-view car racing game |
| Kart Racer | :white_circle: | Power-up kart racing action |
| Time Trial | :white_circle: | Race against the clock solo |
| Endless Driving | :white_circle: | Procedural infinite road driving |
| Drift Racer | :white_circle: | Score points with controlled drifts |
| Motorcycle Racer | :white_circle: | Two-wheeled high-speed racing |
| Obstacle Race | :white_circle: | Dodge hazards on the track |
| Boat Racing | :white_circle: | Water-based racing with waves |
| Hovercraft Racing | :white_circle: | Frictionless hovercraft racing game |
| Rally Racing | :white_circle: | Off-road terrain rally stages |

## Card / Board

| Game | Status | Description |
|------|--------|-------------|
| Solitaire | :white_check_mark: | Classic Klondike card solitaire |
| Poker | :white_check_mark: | Texas Hold'em poker game |
| Blackjack | :white_check_mark: | Hit or stand to beat dealer |
| Chess | :white_check_mark: | Full chess with move validation |
| Checkers | :white_check_mark: | Diagonal capture board game |
| Tic-Tac-Toe | :white_check_mark: | Classic X and O on a 3x3 grid |
| Connect 4 | :white_check_mark: | Drop discs to connect four |
| Reversi | :white_circle: | Flip opponent pieces on board |
| Deckbuilder | :white_circle: | Slay the Spire-style card combat |
| Trading Card Game | :white_circle: | Collectible card battle system |
| Turn-based Board Game | :white_circle: | Digital board game with dice |

## Sandbox

| Game | Status | Description |
|------|--------|-------------|
| Terraria-like | :white_circle: | Side-view dig and build world |
| 2D Minecraft-style | :white_circle: | Block-based crafting sandbox game |
| Physics Sandbox | :white_circle: | Freeform physics object playground |
| Construction Sandbox | :white_circle: | Build structures with various materials |
| Particle Sandbox | :white_circle: | Interactive particle system playground |
| Ecosystem Sandbox | :white_circle: | Create and observe living ecosystems |
| World Generator | :white_circle: | Procedural world creation tool |
| Procedural Exploration | :white_circle: | Explore infinitely generated terrain |
| Creative Builder | :white_circle: | Freeform creative building canvas |
| Mod Sandbox | :white_circle: | User-scriptable modding playground |

## Physics-based

| Game | Status | Description |
|------|--------|-------------|
| Angry Birds Style | :white_check_mark: | Slingshot projectiles at structures |
| Rope Physics | :white_circle: | Swing on realistic rope mechanics |
| Bridge Builder | :white_circle: | Engineer bridges to support weight |
| Ragdoll Playground | :white_circle: | Floppy character physics sandbox |
| Destruction Sim | :white_circle: | Demolish structures with physics |
| Catapult Game | :white_circle: | Launch projectiles at distant targets |
| Marble Physics | :white_circle: | Roll marbles through obstacle courses |
| Platform Physics Puzzles | :white_circle: | Physics-driven platforming challenges |
| Domino Simulation | :white_circle: | Set up and topple domino chains |
| Balance Puzzles | :white_circle: | Stack and balance objects carefully |

## Survival

| Game | Status | Description |
|------|--------|-------------|
| Zombie Survival | :white_check_mark: | Fight off waves of zombies |
| Wave Defense | :white_check_mark: | Defend position against enemy waves |
| Crafting Survival | :white_circle: | Gather materials and craft tools |
| Procedural Island Survival | :white_circle: | Survive on a random island |
| Horde Survival | :white_circle: | Endless enemy horde endurance game |
| Base Survival | :white_circle: | Build and defend survival base |
| Day/Night Survival | :white_circle: | Survive with day-night cycle |
| Hunger System | :white_circle: | Manage hunger and find food |
| Weather Survival | :white_circle: | Endure harsh weather conditions |
| Roguelite Survival | :white_circle: | Permadeath survival with unlocks |

## Educational

| Game | Status | Description |
|------|--------|-------------|
| Typing Games | :white_check_mark: | Improve typing speed and accuracy |
| Math Puzzles | :white_check_mark: | Solve math problems under pressure |
| Programming Puzzles | :white_circle: | Learn coding through puzzle solving |
| Language Learning | :white_circle: | Interactive vocabulary practice game |
| Logic Training | :white_circle: | Sharpen logical reasoning skills |
| Memory Card | :white_check_mark: | Flip cards to find matching pairs |
| Simon | :white_check_mark: | Repeat growing color sequences |
| Memory Games | :white_circle: | Test and improve memory recall |
| Pattern Recognition | :white_circle: | Identify and complete visual patterns |
| Whack-a-Mole | :white_check_mark: | Click moles before they hide |
| Speed Reaction | :white_check_mark: | Fast reflexes reaction time game |
| Quiz Games | :white_check_mark: | Trivia and knowledge quiz game |
| Geography Games | :white_circle: | Learn world geography interactively |

## Experimental

| Game | Status | Description |
|------|--------|-------------|
| Rhythm Game | :white_check_mark: | Hit notes to the music beat |
| Conway's Game of Life | :white_check_mark: | Cellular automaton life simulation |
| Music Visualizer | :white_circle: | Reactive visuals driven by audio |
| AI NPC Sandbox | :white_circle: | Autonomous NPC behavior playground |
| Procedural Storytelling | :white_circle: | Generated narrative branching stories |
| Evolution Simulation | :white_circle: | Natural selection and mutation sim |
| Cellular Automata | :white_circle: | Custom rule cellular automata explorer |
| Art Generator | :white_circle: | Procedural and generative art tool |
| Generative Puzzles | :white_circle: | Procedurally generated unique puzzles |
| Algorithm Visualizer | :white_circle: | Animated sorting and pathfinding demos |

## Tools

| Game | Status | Description |
|------|--------|-------------|
| Playground | :white_check_mark: | Browser IDE with Monaco editor |

---

## Contributing

PRs are welcome for any **planned** or **in progress** game. To get started:

1. Pick a game marked with :white_circle: or :white_check_mark: from the list above
2. Each example should follow the **standard example structure** used by existing games in this repo
3. Open a PR against this repo and reference the game name from the roadmap

For engine questions, bugs, or feature requests, head to the main **[Cubeforge repo](https://github.com/1homsi/cubeforge)**.
